...
def equipment_numerical_status_page(request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    for item in items:
        ...
        mg["total"] += 1; mg["theoretical"] = max(mg["theoretical"], theoretical)
    ...
    def add_percentages(stats, parent_total=None):
        base = stats["total"] or 0; parent = parent_total or 0
    ...
    for category_name, cg in sorted(groups.items(), key=lambda x: (x[1]["sort"], x[0])):
        ...
        for type_name, tg in sorted(cg["types"].items()):
            ...
            ts = sum_stats([m["stats"] for m in model_rows]); ts["need"] = max(0, ts["theoretical"] - ts["total"])
            type_rows.append({"name": type_name, "stats": ts, "models": model_rows})
        cs = sum_stats([t["stats"] for t in type_rows]); cs["need"] = max(0, cs["theoretical"] - cs["total"])
        hierarchy.append({"name": category_name, "stats": cs, "types": type_rows})

    totals = sum_stats([c["stats"] for c in hierarchy]); totals["need"] = max(0, totals["theoretical"] - totals["total"]); totals["equipment"] = totals["total"]
