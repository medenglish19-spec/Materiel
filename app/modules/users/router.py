...
هنا أي منطق عمل (لا تشفير، لا تحقق...) - هذا من مسؤولية services.py.
"""
...
async def login_submit(request: Request, db: Session = Depends(get_db)):
    ...
    response.set_cookie(key=settings.SESSION_COOKIE_NAME, value=token, httponly=True, max_age=settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60)
    return response
...
def users_page(
    ...
    return templates.TemplateResponse("users.html", {"request": request, "users": services.list_users(db)})


@router.get("/api/users", response_model=list[UserOut])
def api_list_users(db: Session = Depends(get_db), _: User = Depends(require_role(Role.ADMIN))):
    return services.list_users(db)


@router.post("/api/users", response_model=UserOut, status_code=status.HTTP_201_CREATED)
def api_create_user(user_in: UserCreate, db: Session = Depends(get_db), _: User = Depends(require_role(Role.ADMIN))):
    try:
...
def api_update_user(user_id: int, user_in: UserUpdate, db: Session = Depends(get_db), _: User = Depends(require_role(Role.ADMIN))):
