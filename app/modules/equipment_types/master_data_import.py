...
def import_master_data(db: Session, content: bytes) -> dict[str, int]:
    ...
    with db.begin():
        for n, row in enumerate(_rows(wb["Categories"]), 2):
            code = str(_require(row, "code", "Categories", n))
            name = str(_require(row, "name", "Categories", n))
            sort_order = _int(_value(row, "sort_order", 0), "sort_order", "Categories", n, 0) or 0
            category_ids[code] = _upsert_by_name(db, "equipment_categories", name, {"code": code, "sort_order": sort_order})
            counts["categories"] += 1

        for n, row in enumerate(_rows(wb["Brands"]), 2):
            name = str(_require(row, "name", "Brands", n))
            brand_ids[name] = _upsert_by_name(db, "equipment_brands", name, {"is_active": True})
            counts["brands"] += 1

        for n, row in enumerate(_rows(wb["Types"]), 2):
            name = str(_require(row, "name", "Types", n))
            unit = str(_require(row, "measurement_unit", "Types", n))
            category_code = str(_require(row, "category_code", "Types", n))
            if category_code not in category_ids:
                raise ValueError(f"Types: الصف {n}: category_code غير موجود: {category_code}")
            quantity = _int(_value(row, "theoretical_quantity"), "theoretical_quantity", "Types", n, 0)
        ...
        for sheet, config_type in (("TirePositions", "TIRES"), ("BatteryConfigurations", "BATTERY")):
            ...
            for n, row in enumerate(_rows(wb[sheet]), 2):
                code = str(_require(row, "config_code", sheet, n))
                name = str(_require(row, "config_name", sheet, n))
                cid = _config(db, code, config_type, name)
                config_ids[code] = cid
                item_code = str(_require(row, "item_code", sheet, n))
                data = {
                    "item_code": item_code,
                    "item_name": str(_require(row, "item_name", sheet, n)),
                    "axle_number": _int(_value(row, "axle_number"), "axle_number", sheet, n, 1),
                    "side": _value(row, "side"),
                    "position_type": _value(row, "position_type"),
                    "sort_order": _int(_value(row, "sort_order", 0), "sort_order", sheet, n, 0) or 0,
                    "value_text": _value(row, "value_text"),
                    "value_number": _float(_value(row, "value_number"), "value_number", sheet, n),
                    "unit": _value(row, "unit"),
                    "extra_json": _value(row, "extra_json"),
                }
        ...
        if "ModelProperties" in sheets:
            ...
            for n, row in enumerate(_rows(wb["ModelProperties"]), 2):
                key = (str(_require(row, "model_name", "ModelProperties", n)), str(_require(row, "type_name", "ModelProperties", n)), str(_require(row, "brand_name", "ModelProperties", n)))
                model_id = model_ids.get(key)
                if model_id is None:
                    raise ValueError(f"ModelProperties: الصف {n}: الطراز غير موجود في Models")
