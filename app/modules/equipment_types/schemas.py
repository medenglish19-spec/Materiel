...
class EquipmentTypeUpdate(BaseModel):
    ...
    def measurement_unit_valid(cls, v: str) -> str:
        if v not in MEASUREMENT_UNITS:
            raise ValueError("وحدة القياس غير صحيحة")
        return v
...
class TirePositionInput(BaseModel):
    ...
    def side_valid(cls, v: str) -> str:
        if v not in POSITION_SIDES:
            raise ValueError("جهة الموضع غير صالحة")
        return v
    ...
    def type_valid(cls, v: str) -> str:
        if v not in POSITION_TYPES:
            raise ValueError("نوع الموضع غير صالح")
        return v
    ...
    def axle_valid(cls, v: int) -> int:
        if v < 1:
            raise ValueError("رقم المحور غير صالح")
        return v
...
class EquipmentModelCreate(BaseModel):
    ...
    def counts_valid(cls, v: int) -> int:
        if v < 0:
            raise ValueError("عدد التجهيزات لا يمكن أن يكون سالبًا")
        return v
    ...
    def axle_count_valid(cls, v: Optional[int]) -> Optional[int]:
        if v is not None and v < 1:
            raise ValueError("عدد المحاور يجب أن يكون رقمًا موجبًا")
        return v
    ...
    def positive_specs(cls, v: Optional[float]) -> Optional[float]:
        if v is not None and v <= 0:
            raise ValueError("قيمة البطارية يجب أن تكون أكبر من صفر")
        return v

    @field_validator("mobility_type")
    @classmethod
    def mobility_valid(cls, v: str) -> str:
        if v not in MOBILITY_TYPES:
            raise ValueError("يجب تحديد ما إذا كان العتاد متحركًا أو مجرورًا")
        return v
