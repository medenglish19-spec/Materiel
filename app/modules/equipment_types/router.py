...
def set_type_theoretical_quantity_form(type_id: int, theoretical_quantity: int = Form(...), db: Session = Depends(get_db), current_user: User = Depends(require_role(Role.ADMIN))):
    obj = services.get_type(db, type_id)
    if obj:
        try:
            services.set_type_theoretical_quantity(db, obj, theoretical_quantity)
        except ValueError:
            pass
    return RedirectResponse(url="/equipment-types", status_code=status.HTTP_302_FOUND)
...
def create_model_form(name: str = Form(...), equipment_type_id: int = Form(...), brand_id: int = Form(...), db: Session = Depends(get_db), current_user: User = Depends(require_role(Role.ADMIN))):
    try:
        services.create_model(db, EquipmentModelCreate(name=name, equipment_type_id=equipment_type_id, brand_id=brand_id))
    except (ValueError, TypeError):
        pass
    return RedirectResponse(url="/equipment-types", status_code=status.HTTP_302_FOUND)
