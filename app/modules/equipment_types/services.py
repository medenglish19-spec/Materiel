...
def create_model(db:Session,data:EquipmentModelCreate)->EquipmentModel:
    ...
    if not data.has_tires and data.tire_positions_required!=0: raise ValueError("عدد مواضع الإطارات غير صحيح")
    if data.has_batteries and data.battery_count_required<1: raise ValueError("هذا الطراز يملك بطاريات؛ يجب تحديد عدد البطاريات")
    if not data.has_batteries and data.battery_count_required!=0: raise ValueError("عدد البطاريات غير صحيح")
    name=data.name.strip()
    if not name: raise ValueError("اسم الطراز مطلوب")
    if db.query(EquipmentModel).filter(EquipmentModel.equipment_type_id==data.equipment_type_id,EquipmentModel.name==name,EquipmentModel.brand_id==data.brand_id).first(): raise ValueError("الطراز موجود مسبقًا لهذا النوع والعلامة")
    obj=EquipmentModel(name=name,equipment_type_id=data.equipment_type_id,brand_id=data.brand_id,has_tires=data.has_tires,tire_positions_required=data.tire_positions_required,has_batteries=data.has_batteries,battery_count_required=data.battery_count_required,mobility_type=data.mobility_type,requires_driver=data.requires_driver)
