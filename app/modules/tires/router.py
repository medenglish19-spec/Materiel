...
def _model_tire_configuration(db: Session):
    ...
    for model in models:
        configurations[model.id] = {"has_tires": bool(model.has_tires), "tire_positions_required": model.tire_positions_required, "tire_size": model.tire_size, "positions": [{"id": p.id, "name": p.name, "code": p.code} for p in services.list_positions(db, model.id)], "sizes": [s.size for s in db.query(TireModelSize).filter(TireModelSize.equipment_model_id == model.id).order_by(TireModelSize.id).all()]}
