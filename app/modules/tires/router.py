...
def tire_detail(request: Request, tire_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("tire_detail.html", {"request": request, "user": current_user, "tire": tire, "state": state, "condition": services.tire_condition(tire, state), "history": services.movement_history(db, tire_id), "equipment": db.query(Equipment).order_by(Equipment.registration_number, Equipment.id).all(), "positions": services.list_positions(db), "today": date.today()})
