...
def status_for(rule, equipment, record, current_value):
    ...
    due_value = None
    remaining = None
    if rule.interval_km is not None and current_value is not None and record.meter_value is not None:
        due_value = Decimal(str(record.meter_value)) + Decimal(str(rule.interval_km))
        remaining = due_value - Decimal(str(current_value))
        if remaining <= 0:
            return "مستحقة الآن", "danger", remaining
        if rule.warning_km is not None and remaining <= Decimal(str(rule.warning_km)):
...
def periodic_maintenance_page(
    ...
    for eq in equipment:
        ...
        current_value = current.odometer if current and current.odometer is not None else eq.current_odometer
        for rule in rules:
            ...
            counts["total"] += 1
            counts[css] += 1
            next_meter = (Decimal(str(rec.meter_value)) + Decimal(str(rule.interval_km))) if rec and rec.meter_value is not None and rule.interval_km is not None else None
...
def maintenance_rule_create(
    ...
    rule = MaintenanceRule(name=name.strip(), equipment_type_id=equipment_type_id, interval_km=dec(interval_km), interval_hours=dec(interval_hours), interval_days=int(interval_days) if interval_days else None, warning_km=dec(warning_km), warning_days=int(warning_days) if warning_days else None, description=description.strip() or None)
...
def maintenance_record_create(
    ...
    if meter_value:
        ...
        except (InvalidOperation, ValueError): meter = None
    ...
    for eq in equipment:
        ...
        for rule in rules:
            ...
            if state in ("مستحقة الآن", "قريبة", "بلا سجل"):
                due_rows.append({"equipment": eq, "rule": rule, "record": rec, "current": current_value, "remaining": remaining, "state": state, "css": css})
