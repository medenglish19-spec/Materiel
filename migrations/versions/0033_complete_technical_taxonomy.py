"""repair and complete the global technical taxonomy after a partially applied 0032

revision: 0033
down_revision: 0032_technical_spec_dictionary
"""
from alembic import op
import sqlalchemy as sa

revision = "0033_complete_technical_taxonomy"
down_revision = "0032_technical_spec_dictionary"
branch_labels = None
depends_on = None
...
VEHICLE_SPECS = [
    ("الصانع","manufacturer","التعريف الفني","text",None,1,None),
    ("الطراز","model","التعريف الفني","text",None,2,None),
    ("الجيل","generation","التعريف الفني","text",None,3,None),
    ("سنة الصنع","model_year","التعريف الفني","number","سنة",4,None),
    ("بلد الصنع","country_of_origin","التعريف الفني","text",None,5,None),
...
def upgrade():
    ...
    for name, code, sort_order, type_rows in TAXONOMY:
        row = bind.execute(sa.text(
            "SELECT id FROM equipment_categories WHERE code=:code"
        ), {"code": code}).fetchone()
        if row:
            category_id = row[0]
        else:
            category_id = bind.execute(sa.text(
                ...
                "(name,code,sort_order,is_system,created_at,updated_at) "
                "VALUES (:name,:code,:sort_order,1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) "
                "RETURNING id"
            ), {"name": name, "code": code, "sort_order": sort_order}).scalar_one()
        category_ids[code] = category_id

        for type_name, unit in type_rows:
            row = bind.execute(sa.text(
                "SELECT id FROM equipment_types WHERE name=:name"
            ), {"name": type_name}).fetchone()
            if not row:
    ...
    for name, code, group_name, data_type, unit, sort_order, *rest in VEHICLE_SPECS:
        exists = bind.execute(sa.text(
            "SELECT id FROM equipment_model_spec_definitions WHERE name=:name"
        ), {"name": name}).fetchone()
        if exists:
            continue
        options = None
