...
def upgrade() -> None:
    ...
    fks = {
        fk.get("name")
        for fk in inspector.get_foreign_keys("maintenance_rules")
    }
    ...
    with op.batch_alter_table(
        ...
        # The pair is nullable and must remain compatible with existing
        # generic rules. SQLite permits multiple NULLs in a unique constraint.
        if not any(
            tuple(u.get("column_names") or ()) == ("parent_rule_id", "equipment_model_id")
            for u in inspector.get_unique_constraints("maintenance_rules")
        ):
...
def downgrade() -> None:
    ...
    with op.batch_alter_table(
        ...
        for index_name in (
            ...
            if index_name in {i["name"] for i in inspect(bind).get_indexes("maintenance_rules")}:
